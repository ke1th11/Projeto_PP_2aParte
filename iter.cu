...
extern "C" {
    void cuda_init(int np, int nx) {
        ...
        cudaMalloc(&d_E, (nx + 1) * sizeof(t_fld));
        cudaMalloc(&d_B, (nx + 1) * sizeof(t_fld));
        cudaMalloc(&d_J, (nx + 1 + 3) * sizeof(t_fld)); // Espaço extra para segurança
    }
...
__global__ void spec_advance_kernel(t_part *parts, int np, const t_fld *E, const t_fld *B, t_fld *J, int nx, float tem, float dt_dx, float qnx) {
    ...
    // --- 1. LEITURA E INTERPOLAÇÃO DOS CAMPOS ---
    float x = parts[i].x;
    int ix = parts[i].ix;
    float w1 = x;
    float w0 = 1.0f - w1;
    ...
    // --- 2. BORIS PUSHER (Atualização das Velocidades) ---
    float ux = parts[i].ux + tem * Ep.x;
    float uy = parts[i].uy + tem * Ep.y;
    float uz = parts[i].uz + tem * Ep.z;
    ...
    // --- 3. DEPOSIÇÃO DE CORRENTE (O Coração da Física) ---
    float vx = ux * rg;
    float vy = uy * rg;
    float vz = uz * rg;
    float dx = dt_dx * vx;

    // Componentes transversais (Y e Z)
    atomicAdd(&J[ix].y,   qnx * vy * w0);
    atomicAdd(&J[ix+1].y, qnx * vy * w1);
    atomicAdd(&J[ix].z,   qnx * vz * w0);
    atomicAdd(&J[ix+1].z, qnx * vz * w1);

    // Componente longitudinal (X) - Deposição linear simplificada
    float x_mid = x + 0.5f * dx;
...
extern "C" void spec_advance_cuda(t_part *h_parts, int np, const t_fld *h_E, const t_fld *h_B, void *h_J, int nx, float tem, float dt_dx, float qnx) {
    ...
    size_t size_p = np * sizeof(t_part);
    size_t size_f = (nx + 1) * sizeof(t_fld);
    // A corrente no CPU tem guard cells, mas aqui tratamos o buffer passado
    size_t size_j = (nx + 2) * sizeof(t_fld);

    cudaMemcpy(d_parts, h_parts, size_p, cudaMemcpyHostToDevice);
    cudaMemcpy(d_E, h_E, size_f, cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, h_B, size_f, cudaMemcpyHostToDevice);

    // Limpamos o buffer de corrente na GPU antes de cada iteração
    cudaMemset(d_J, 0, size_j);

    int tpb = 256;
    int bpg = (np + tpb - 1) / tpb;
    spec_advance_kernel<<<bpg, tpb>>>(d_parts, np, d_E, d_B, (t_fld*)d_J, nx, tem, dt_dx, qnx);

    // 1. Esperar a GPU acabar
    cudaDeviceSynchronize(); 

    // 2. Copiar as partículas (para o loop de energia do CPU)
    cudaMemcpy(h_parts, d_parts, size_p, cudaMemcpyDeviceToHost);

    // 3. Copiar a corrente (para a física dos campos)
    // Copiamos nx + 1 para garantir que pegamos a borda direita
    cudaMemcpy(h_J, d_J, (nx + 1) * sizeof(t_fld), cudaMemcpyDeviceToHost);
}
*/
    ...
    int nx, float tem, float dt_dx, float qnx) 
{
    ...
    // Jx
    atomicAdd(&J[ix].x,   qnx * vx * w0);
    atomicAdd(&J[ix+1].x, qnx * vx * w1);
    // Jy
    atomicAdd(&J[ix].y,   qnx * vy * w0);
    atomicAdd(&J[ix+1].y, qnx * vy * w1);
    // Jz
...
/*
extern "C"
void spec_advance_cuda(
    t_part *h_parts, int np,
    const t_fld *h_E, const t_fld *h_B,
    int nx, float tem, float dt_dx)
{
    if (!cuda_initialized) {
        cuda_init(np, nx);
    ...
    cudaMemcpy(d_parts, h_parts, np * sizeof(t_part),
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_E, h_E, (nx + 1) * sizeof(t_fld),
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, h_B, (nx + 1) * sizeof(t_fld),
               cudaMemcpyHostToDevice);

    int tpb = 256;
    int bpg = (np + tpb - 1) / tpb;

    spec_advance_kernel<<<bpg, tpb>>>(
        d_parts, np, d_E, d_B, nx, tem, dt_dx);

    cudaDeviceSynchronize();

    cudaMemcpy(h_parts, d_parts, np * sizeof(t_part),
